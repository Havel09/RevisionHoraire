...
function createCreneaux($classesId, $coursId, $jour, $heureDebut, $heureFin, $salle)
{
    try {
        $db = getDb();
        $stmt = $db->prepare(
            "INSERT INTO creneaux(classes_id, cours_id, jour, heure_debut, heure_fin, salle)
        VALUES (:classes_id, :cours_id, :jour, :heure_debut, :heure_fin, :salle)
        "
        );

        $stmt->bindParam(":classes_id", $classesId);
...
function updateCreneaux($classesId, $coursId, $jour, $heureDebut, $heureFin, $salle)
{
    try {
        $db = getDb();
        $stmt = $db->prepare(
            "UPDATE creneaux SET classes_id = :classes_id, cours_id = :cours_id, jour = :jour, heure_debut = :heure_debut, heure_fin = :heure_fin, salle = :salle WHERE id = :id"
        );
